...
/**
 *	\file       htdocs/core/modules/supplier_invoice/pdf/pdf_coco.modules.php
 *	\ingroup    fournisseur
 *	\brief      Class file to generate the supplier invoices with the coco model
 */
...
/**
 *	\class      pdf_coco
 *	\brief      Class to generate the supplier invoices with the coco model
 */
class pdf_coco extends ModelePDFSuppliersInvoices
{
    var $db;
    var $name;
    var $description;
	...
	function pdf_coco($db,$object)
	{
		...
		$this->description = $langs->trans('SuppliersInvoiceModelCoco');
	...
	function write_file($object,$outputlangs='')
	{
		...
		if ($conf->fournisseur->dir_output.'/facture')
		{
			...
			else
			{
                ...
                $objectref = dol_sanitizeFileName($object->ref_ext);
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetTitle($outputlangs->convToOutputCharset($object->ref_ext));
				$pdf->SetSubject($outputlangs->transnoentities("Invoice"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref_ext)." ".$outputlangs->transnoentities("Order"));
	...
	function _pagehead(&$pdf, $object, $showaddress=1, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("SupplierInvoice")." ".$outputlangs->convToOutputCharset($object->ref_ext), '', 'R');
		$pdf->SetFont('','', $default_font_size + 2);
