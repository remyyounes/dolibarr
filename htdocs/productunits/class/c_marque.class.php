...
/**
 *      \file       dev/skeletons/c_marque.class.php
 *      \ingroup    mymodule othermodule1 othermodule2
 *      \brief      This file is an example for a CRUD class file (Create/Read/Update/Delete)
 *		\version    $Id: c_marque.class.php,v 1.29 2010/04/29 14:54:13 grandoc Exp $
 ...
 *		\remarks	Initialy built by build_class_from_table on 2010-08-11 16:25
 */
class C_marque // extends CommonObject
{
	var $db;							//!< To store db handler
	var $error;							//!< To return error code (or message)
	var $errors=array();				//!< To return several error codes (or messages)
	//var $element='c_marque';			//!< Id that identify managed objects
	//var $table_element='c_marque';	//!< Name of table without prefix where object is stored
    ...
    function create($user, $notrigger=0)
    {
        ...
        // Insert request
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."c_marque(";
		...
		if (! $error)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."c_marque");
    ...
    function fetch($id)
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."c_marque as t";
    ...
    function update($user=0, $notrigger=0)
    {
        ...
        // Update request
        $sql = "UPDATE ".MAIN_DB_PREFIX."c_marque SET";
	...
	function delete($user, $notrigger=0)
	{
		...
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."c_marque";
		$sql.= " WHERE rowid=".$this->id;
	...
	/**
	 *    	\brief      Return an html string with a select combo box to choose from load_cache_marquecode
	...
	function select_code($selected='',$htmlname='condid',$addempty=0)
	{
		...
		$this->load_cache_marquecode();
		$input = "";
		$input .= '<select class="flat" name="'.$htmlname.'">';
		if ($addempty) $input .= "<option value=''>&nbsp;</option>";
		foreach($this->cache_marquecode as $id => $arrayconditions)
		{
...
/**
	 ...
	 */
	function load_cache_marquecode()
	{
		global $langs;

		if (sizeof($this->cache_marquecode)) return 0;    // Cache deja charge

		$sql = "SELECT rowid as id, code, libelle";
		$sql.= " FROM ".MAIN_DB_PREFIX."c_marque";
		$sql.= " WHERE active > 0";
		$sql.= " ORDER BY code ASC";
		dol_syslog('c_marque::load_cache_codemarque sql='.$sql,LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql)
		{
			...
			{
				$obj = $this->db->fetch_object($resql);
				$this->cache_marquecode[$obj->id]['code'] =$obj->code;
				$this->cache_marquecode[$obj->id]['label']=$obj->libelle;
				$i++;
			}
			return $num;
		}
